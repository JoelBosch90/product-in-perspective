...
class EventHandler {
  ...
  trigger = (event, data) => {
    ...
    // Trigger the same event on all other handlers we want to bubble to.
    if (this._bubbleTo) for (const handler of this._bubbleTo) handler.trigger(event, data);
  ...
  bubbleTo = handler => {
    ...
    if (!handler instanceof EventHandler) return this;
  ...
  off = (event, listener) => {
    ...
    // remove one.
    if (this._triggers[event] == undefined) return this;
