...
class Logout extends BaseElement {
  ...
  constructor(parent) {
    ...
    // Determine the component's title.
    const title = "Logging out.";

    // Use the component's title as the page title.
    this.pageTitle(title);

    // Show a brief message that we're logging out.
    this._apology = new Apology(this._container, { title: title });

    // We need to make an API call to log off so we need the Request object.
    this._request = new Request();

    this._request.get('/logout')
      .catch(error => void this._apology.update("Error occurred: could not log out."))
      .then(response => {

        // If we get no response, something weird happened and we could not log
        // out.
        if (!response) return this._apology.update("Error occurred: could not log out.");

        // Move to the login page. Here we don't want to use our usual `goTo`
