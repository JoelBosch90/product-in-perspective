...
class FormMultiSelect extends BaseElement {
  ...
  constructor(parent, options = {}) {
    ...
    this._select.on("change", this._checkAddState);

    // Create a container to show the current selections.
  ...
  selectCurrent = event => {
    ...
    // Select these options and allow chaining.
  ...
  reset = event => {
    ...
    // Clear this widget and allow chaining.
    return this.clear();
  }
  ...
  select = (value, label) => {
    ...
    // When there are selections, they can be cleared.
    this._clear.disabled(false);

    // Create a new element to show the newly added selection.
    const selection = document.createElement('p');

    // Add the selection class for custom styling.
    selection.classList.add('selection');

    // Add the label as an indicator of this selection.
    selection.textContent = label;

    // Add the new selection.
    this._selectionDisplay.appendChild(selection);
  ...
  disabled = disable => {
    ...
    this._clear.disabled = disable;

    // Set the request state on the select widget.
  ...
  addOption = (value, label) => {
    ...
    // Store the option in our options array.
  ...
  clear = () => {
    ...
    // Remove all current selections for our display.
    while(this._selectionDisplay.firstChild) this._selectionDisplay.removeChild(this._selectionDisplay.lastChild);
