...
class BaseElement extends EventHandler {
  ...
  pageTitle(newTitle) {
    ...
    // Install it as the page's title.
  ...
  show() {
    ...
    if (this._container) this._container.removeAttribute('hidden');

    // If we have a page title, we want to install it again.
    if (this._pageTitle) this.pageTitle(this._pageTitle);
  ...
  hide() {
    ...
    if (this._container) this._container.setAttribute('hidden', '');
