 ...
 */
 class Reticle extends BaseElement {
  ...
  constructor(parent) {
    ...
    // element in the DOM.
    this._registerPrimitive('ar-hit-test');
    ...
    this._container.classList.add("arscene-reticle");
    ...
    this._container.setAttribute("rotation", "0 0 0");
    ...
    // Add the reticle to the parent element.
  ...
  _registerComponent = (name) => {
    ...
    if (!AFRAME.components[name]) AFRAME.registerComponent(name, {
      ...
      schema: {
        ...
        // tests or not.
        doHitTest: { type: 'boolean', default: true }
      },
      ...
      init: function () {
        ...
        // We want to keep track of whether we've already found a pose or not.
        this.hasFoundAPose = false;

        // Event handler for when the ession ends.
        this.onSessionEnd = () => {
      ...
      tick: function (time, timeDelta) {
        ...
        // Get access to the current frame.
        ...
        // Perform the hit test for this frame.
        const result = this.hitTest.doHit(frame);

        // If there is no result, the hit test failed and we should probably not
        ...
        if (!result) {
          ...
          // Signal a failed hit test.
          reticle._success(false);
        ...
        // Try to get the pose for this frame.
        try { this.currentControllerPose = frame.getPose(inputSpace, this.el.sceneEl.renderer.xr.getReferenceSpace()); }
  ...
  _success = success => {
    ...
    if (this._latestSuccess == success) return;
  ...
  show() {
    ...
    // Make sure that we're performing hit tests.
  ...
  hide() {
    ...
    // We don't need to perfom hit tests while the reticle is hidden.
    this._container.setAttribute("testing", false);
  ...
  remove() {
    ...
    // Call the BaseElement remove method.
    super.remove();
  }
}
