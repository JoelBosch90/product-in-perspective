...
const xrButtonClickHandler = event => {
  ...
  if (!xrSession) {
    ...
    navigator.xr.requestSession(sessionType, { requiredFeatures: ['local', 'hit-test'] }).then(session => {
...
const xrSessionStartHandler = () => {
  ...
  xrSession.addEventListener('select', xrSessionSelectHandler);

  // @TODO Do we need to remove a background here?

  // Create a new canvas element.
  const canvas = document.createElement('canvas');

  // Get the WebGL rendering context.
  renderingContext = canvas.getContext('webgl', { xrCompatible: true });
  ...
  xrSession.requestReferenceSpace('viewer').then(referenceSpace => {
    ...
    // Request and store the hit test source.
...
const xrSessionSelectHandler = event => {
  ...
  // @TODO add the AR object at the reticle.matrix position.
};
...
const xrFrameHandler = (time, frame) => {
  ...
  // Get the XR viewer pose from the reference space.
  const viewerPose = frame.getViewerPose(xrReferenceSpace);

  // We only want to draw the reticle when we have a valid hit test. That is why
  // the reticle is hidden by default.
  // @TODO Apply to actual reticle.
  reticle.visible = false;

  // Did we get a valid XR viewer pose? It could be lost or blocked. In that
  ...
  // Did we get a valid hit source for this session?
  if (xrHitTestSource) {

    // Get the hit test results for this XR frame.
    const hitTestResults = frame.getHitTestResults(xrHitTestSource);

    // Did we get a valid hit for the test?
    if (hitTestResults.length > 0) {
      ...
      // Now we can show the reticle.
      // @TODO Apply to actual reticle.
      reticle.visible = true;

      // Apply the hit test transformation matrix to the reticle.
      // @TODO Apply to actual reticle.
  ...
  // We need to provide the WebGL rendering context with the framebuffer.
  renderingContext.bindFramebuffer(renderingContext.FRAMEBUFFER, glLayer.framebuffer);

  // Iterate over the views in the XR viewer pose. Mobile devices have only the
  ...
  for (const view of viewerPose.views) {
    ...
    // Provide the viewport to the WebGL rendering context.
