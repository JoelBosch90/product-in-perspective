...
class HitTest {
  ...
  constructor(renderer, options) {
    console.log("HitTest::constructor"); // Store the XR renderer.
  ...
  doHit(frame) {
    console.log("HitTest::doHit"); // If we're not in an active session, we cannot do a hit test.
  ...
  _hitTest(frame, referenceSpace) {
    ...
    if (results.length <= 0) return false; // Get the pose of the first result of the hit test. This should be the one
