...
const scanner = new BarcodeScanner(document.body.querySelector('#barcodescanner-view'));
...
scanner.on('scanned', data => {
  ...
  if (data.code in products) {
    ...
    lastProduct = products[data.code];
...
const switchMode = (element, mode) => {
  ...
  console.log("::switchMode", element, mode);

  // Get the classList of the element we want to switch to a different mode.
  const classList = element.classList;

  // Remove all possible modes from this classList.
  ["scanning", "placing", "viewing"].forEach(mode => void classList.remove(mode));

  // Add only the new mode.
  classList.add(mode);
};
...
const inMode = (element, mode) => {
  ...
  // Simply check if the classList contains the requested mode.
...
const initVR = () => {
  ...
  // Log an error.
  console.error("VR mode is not yet supported.");
...
const initAR = () => {
  ...
  // @TODO figure out how this works. It looks like this has something to do
  // with the overlay buttons.
  document.getElementById('ar-interface')
    .setAttribute('text', 'value', 'Overlay: ' + hasDOMOverlay(scene.xrSession));
...
const enterScanningMode = () => {
  ...
  // Make sure the parent element is in scanning mode.
  switchMode(document.body, "scanning");

  // We don't need the hit test on the reticle for the scanning mode.
  reticle.setAttribute('ar-hit-test', 'doHitTest:false');

  // Make sure the AR elements are not visible.
...
const enterPlacingMode = () => {
  ...
  // Make sure the parent element is in placing mode.
  switchMode(document.body, "placing");

  // We need the hit test on the reticle for the placing mode.
  reticle.setAttribute('ar-hit-test', 'doHitTest:true');

  // Make sure the reticle is visible.
  reticle.setAttribute('visible', 'true');

  // Make sure the box is not visible.
  box.setAttribute('visible', 'false');

  // Update the text on the progress button.
  progressButton.textContent = "Place";
...
const enterViewingMode = () => {
  ...
  switchMode(document.body, "viewing");
  ...
  // We can copy the position of the reticle to place the box in the scene.
  ...
  box.object3D.quaternion.multiplyQuaternions(tempQuaternion, reticle.object3D.quaternion);

  // Make sure the box is visible.
  box.setAttribute('visible', 'true');

  // Update the text on the progress button.
  progressButton.textContent = "Scan again";
...
const enterNextMode = () => {
  ...
  // If we're placing, we want to finish the hit test and view the
  // representation object.
  if (inMode(parent, "placing")) return enterViewingMode();

  // If we're viewing, we want to start over and scan a new product.
  if (inMode(parent, "viewing")) return scene.xrSession.end();
}

// Reset the scene after we exit XR mode.
scene.addEventListener("exit-vr", event => void enterScanningMode());

// Prepare the scene once we enter XR mode.
scene.addEventListener("enter-vr", event => {

  console.log("@enter-vr");

  // We can only initialize the augmented reality session if the scene has
  // successfully entered AR mode.
  if (scene.is("ar-mode")) return initAR();

  // We can only initialize the augmented reality session if the scene has
  // successfully entered AR mode.
  if (scene.is("vr-mode")) return initVR();

  // If we end up here, something went wrong. We should inform the user and end
  // the session.
...
exitButton.addEventListener('click', function () { scene.xrSession.end(); });

// Make sure users don't accidentally go through the phases too quickly.
const debouncedNextMode = debounce(enterNextMode, 500);

// Register button clicks.
progressButton.addEventListener('mousedown', debouncedNextMode);
progressButton.addEventListener('touchstart', debouncedNextMode);
