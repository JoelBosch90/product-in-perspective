...
class Menu extends BaseElement {
  ...
  constructor(parent, options = {}) {
    ...
    this._addAnchor(shortcuts, options.shortcuts); // Start listening for URL changes.


    window.addEventListener('popstate', this._processPathChange); // App both menu components to the container.
  ...
  _addAnchor = (parent, entries) => {
    ...
    for (const [text, location] of entries) {
      ...
      anchor.classList.add("button"); // Add the text.

      anchor.textContent = text; // Navigate to the right page when this anchor is clicked.

      anchor.addEventListener("click", () => void goTo(location)); // Add the anchor to the navigation menu.
