...
class ArModel extends BaseElement {
  ...
  constructor(parent, ...options) {
    ...
    this._container.classList.add('ar-model');
  ...
  position = (position, rotation) => {
    ...
    // Get the number of models in a single row.
    const row = Math.ceil(Math.pow(number, 1/3));
    ...
    for (let i = 0; i < this._models.length; i++) {
      ...
      // We want to change the y coordinate every floor and we don't need to
      // reset.
      const yLevel = ~~(i / floor);

      // We want to change the z coordinate every row and we need to reset every
      // floor.
      ...
      model.setAttribute("position", update);
  ...
  show() {
    ...
    // We also need to seperately hide all of our models.
    for (const model of this._models) model.setAttribute('visible', 'true');
  ...
  hide() {
    ...
    // We also need to seperately hide all of our models.
    for (const model of this._models) model.setAttribute('visible', 'false');
  ...
  remove() {
    ...
    // Remove all models.
    for (const model of this._models) model.remove();

    // Reset the array.
    this._models = [];

    // Call the BaseHandler's remove function.
    super.remove();
  }
}
